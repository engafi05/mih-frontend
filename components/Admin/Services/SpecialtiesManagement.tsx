  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow-sm border border-gray-100">
      <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
        <FaStethoscope className="text-indigo-600" /> إدارة الخدمات الطبية
      </h2>

      <form onSubmit={handleAdd} className="flex gap-2 mb-8">
        <input 
          type="text" placeholder="اسم الخدمه الجديده..." value={newName}
      ...
      {loading ? <FaSpinner className="animate-spin mx-auto text-2xl" /> : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {services.map(spec => (
            <div key={spec.ServicesID} className="flex justify-between items-center p-4 bg-gray-50 rounded-lg border group hover:border-indigo-200 transition">
              <span className="font-medium text-gray-700">{spec.ServicesName}</span>
